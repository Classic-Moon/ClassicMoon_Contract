...
use classic_classicmoon::mock_querier::{mock_dependencies, WasmMockQuerier};
...
fn create_pair() {
    ...
    deps = init(deps);
    deps.querier
        .with_classicmoon_factory(&[], &[("uusd".to_string(), 6u8)]);
    let assets = [
        Asset {
...
fn fail_to_create_pair_with_unknown_token() {
    ...
    deps.querier
        .with_classicmoon_factory(&[], &[("uluna".to_string(), 6u8)]);
...
fn reply_only_create_pair() {
    ...
    // register classicmoon pair querier
    deps.querier.with_classicmoon_factory(
        &[(
            &"0000".to_string(),
            &PairInfo {
