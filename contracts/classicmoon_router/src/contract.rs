...
use classic_classicmoon::asset::{Asset, AssetInfo, PairInfo};
use classic_classicmoon::pair::{QueryMsg as PairQueryMsg, SimulationResponse};
use classic_classicmoon::querier::{query_pair_info, reverse_simulate};
use classic_classicmoon::router::{
...
const CONTRACT_NAME: &str = "crates.io:classicmoon-router";
...
pub fn instantiate(
    ...
    CONFIG.save(
        deps.storage,
        &Config {
            classicmoon_factory: deps.api.addr_canonicalize(&msg.classicmoon_factory)?,
        },
    )?;
    ...
    match msg {
        ...
        ExecuteMsg::AssertMinimumReceive {
            ...
            assert_minimum_receive(
                ...
                prev_balance,
                minimum_receive,
                //TODO: deps.api.addr_validate(&receiver)?,
...
fn optional_addr_validate(api: &dyn Api, addr: Option<String>) -> StdResult<Option<Addr>> {
    let addr = if let Some(addr) = addr {
        Some(Addr::unchecked(addr))
        //TODO: Some(api.addr_validate(&addr)?)
    } else {
        None
    };
...
pub fn query_config(deps: Deps<TerraQuery>) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        classicmoon_factory: deps
            .api
            .addr_humanize(&state.classicmoon_factory)?
            .to_string(),
    };
...
fn simulate_swap_operations(
    ...
    for operation in operations.into_iter() {
        ...
        offer_amount = match operation {
            ...
            SwapOperation::ClassicmoonSwap {
                offer_asset_info,
                ask_asset_info,
            } => {
                let classicmoon_factory = deps.api.addr_humanize(&config.classicmoon_factory)?;
